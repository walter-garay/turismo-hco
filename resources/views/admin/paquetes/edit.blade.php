        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <!-- Formulario para editar paquete turístico -->
                    <form action="{{ route('admin.paquetes.update', $paquete->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="text" name="nombre" id="nombre" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('nombre', $paquete->nombre) }}" required>
                                @error('nombre')
                            ...
                            <div>
                                ...
                                <input type="number" name="precio_individual" id="precio_individual" step="0.01" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('precio_individual', $paquete->precio_individual) }}" required>
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            <div>
                                ...
                                <input type="number" name="duracion" id="duracion" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" value="{{ old('duracion', $paquete->duracion) }}" required>
                                @error('duracion')
                                    <span class="text-red-600 text-sm">{{ $message }}</span>
                                @enderror
                            </div>
                            <div>
                                ...
                                <select name="destino_id" id="destino_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm" required>
                                    ...
                                    @foreach($destinos as $destino)
                                        <option value="{{ $destino->id }}" {{ $paquete->destino_id == $destino->id ? 'selected' : '' }}>{{ $destino->nombre }}</option>
                                    @endforeach
                                </select>
                                @error('destino_id')
                                    <span class="text-red-600 text-sm">{{ $message }}</span>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="descripcion" id="descripcion" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('descripcion', $paquete->descripcion) }}</textarea>
                            @error('descripcion')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                                    ...
                                    @if($destino->actividades->isNotEmpty())
                                        ...
                                        @foreach($destino->actividades as $actividad)
                                            <div class="flex items-start">
                                                <!-- Marcar actividades seleccionadas en el paquete -->
                                                <input type="checkbox" name="actividades[]" value="{{ $actividad->id }}" id="actividad-{{ $actividad->id }}"
                                                    class="mt-1"
                                                    {{ in_array($actividad->id, $paquete->actividades->pluck('id')->toArray()) ? 'checked' : '' }}>
